...
exports.BUILTIN_TABLES = void 0;
exports.listUniformTables = listUniformTables;
exports.getUniformTable = getUniformTable;
exports.selectUniformTable = selectUniformTable;
exports.generateUniformDesign = generateUniformDesign;
exports.generateCustomUniformTable = generateCustomUniformTable;
 ...
 */
function generateUniformTable(n, generators) {
    const matrix = [];
    for (let i = 1; i <= n; i++) {
...
// ─── 均匀表注册表 ─────────────────────────────────────────
const UNIFORM_TABLES = [U5, U7, U8, U9, U11, U12, U13];
/** 获取所有可用均匀表 */
function listUniformTables() {
    return UNIFORM_TABLES.map(t => ({ ...t, matrix: [] }));
}
/** 按名称获取均匀表 */
function getUniformTable(name) {
    return UNIFORM_TABLES.find(t => t.name === name) ?? null;
}
/** 自动选择合适的均匀表 */
function selectUniformTable(factorCount, minRuns) {
 ...
 */
function generateUniformDesign(factors, tableName) {
    if (factors.length === 0) {
    ...
    if (tableName) {
        ...
        if (!table)
            throw new Error(`未找到均匀表: ${tableName}`);
    }
    else {
    ...
    if (factors.length > table.maxFactors) {
        throw new Error(`因素数(${factors.length})超过均匀表${table.name}的最大列数(${table.maxFactors})`);
    }
    // 分配因素到列
    ...
    // 生成试验方案
    const n = table.runs;
    const runs = table.matrix.map((row, i) => {
        const factorValues = {};
        const factorLevels = {};
        factors.forEach((f, j) => {
 ...
 */
function generateCustomUniformTable(n, factorCount) {
...
// ─── 导出内置表常量 (测试用) ─────────────────────────────
exports.BUILTIN_TABLES = { U5, U7, U8, U9, U11, U12, U13 };
//# sourceMappingURL=doe-uniform.js.map