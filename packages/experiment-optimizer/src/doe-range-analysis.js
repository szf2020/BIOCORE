...
exports.rangeAnalysis = rangeAnalysis;
exports.compositeScore = compositeScore;
exports.multiIndicatorRangeAnalysis = multiIndicatorRangeAnalysis;
exports.analyzeInteraction = analyzeInteraction;
exports.snRatioAnalysis = snRatioAnalysis;
// ─── 单指标极差分析 ─────────────────────────────────────────
/**
 * 正交试验极差分析 (单指标)
 *
 ...
 */
function rangeAnalysis(design, results, factors, goal = 'maximize') {
 ...
 */
function compositeScore(multiResults, weights) {
    if (multiResults.length === 0 || weights.length === 0)
        return [];
    // 计算各指标的 min/max
    const ranges = {};
 ...
 * 先算综合评分, 再用综合评分做极差分析.
 */
function multiIndicatorRangeAnalysis(design, multiResults, factors, weights) {
    const scores = compositeScore(multiResults, weights);
    // 转换为单指标结果 (用综合评分代替)
    const singleResults = multiResults.map((r, i) => ({
        runIndex: r.runIndex,
 ...
 */
function analyzeInteraction(design, results, factorA, factorB) {
    const responseMap = new Map();
    for (const r of results)
        responseMap.set(r.runIndex, r.response);
    ...
    // 均值表
    const means = sums.map((row, i) => row.map((s, j) => counts[i][j] > 0 ? s / counts[i][j] : 0));
    // 交互效应: 偏差平方和
 ...
 */
function computeSN(values, type) {
 ...
 * @param type - S/N 类型
 */
function snRatioAnalysis(design, results, factors, type = 'larger_better') {
    // 计算每次试验的 S/N 比
    ...
    const snMap = new Map();
    for (const sv of snValues)
        snMap.set(sv.runIndex, sv.sn);
...
//# sourceMappingURL=doe-range-analysis.js.map