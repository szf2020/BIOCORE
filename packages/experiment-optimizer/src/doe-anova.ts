 ...
 * 覆盖正交试验常见的自由度组合
 */
const F_TABLE: Record<string, [number, number]> = {
  // df1=1
 ...
 */
function lookupF(df1: number, df2: number): [number, number] {
  // 精确查找
  const key = `${df1}_${df2}`;
 ...
 * 根据 F 值和临界值判断显著性
 */
function getSignificance(F: number, df1: number, df2: number): { significance: '**' | '*' | ''; pValue: number } {
 ...
 */
export function orthogonalAnova(
  design: OrthogonalDesign,
  results: ExperimentResult[],
  factors: DOEFactor[],
  poolThreshold: number = 0.1,
): OrthogonalAnovaResult {
  ...
  // 各因素的 SS 和 df
  const factorSS: { name: string; ss: number; df: number }[] = [];
  ...
  for (const f of factorSS) {
    if (pooledFactors.includes(f.name)) continue; // 已并入误差
 ...
 */
export function orthogonalAnovaWithReplicates(
  design: OrthogonalDesign,
  results: ReplicatedResult[],
  factors: DOEFactor[],
): OrthogonalAnovaResult {
  // 将重复试验展开: 取均值作为各行的代表值, 另计算纯误差
  const avgResults: ExperimentResult[] = results.map(r => ({
    runIndex: r.runIndex,
  ...
  const sources: AnovaSource[] = baseResult.sources.map(s => {
