 ...
 * @param goal - 优化方向
 */
export function createGoldenSearch(
  factorName: string,
  min: number,
  max: number,
 ...
 */
export function advanceGoldenSearch(
 ...
 * 比较两个响应值, 返回 a 是否优于 b
 */
function isBetter(a: number, b: number, goal: OptimizationGoal): boolean {
 ...
 * @returns 全部试验点 (按顺序排列)
 */
export function fibonacciSearchPoints(
  factorName: string,
  min: number,
  max: number,
  totalTrials: number,
): number[] {
  if (totalTrials < 2) return [(min + max) / 2];
 ...
 * @param goal - 优化方向
 */
export function createMultiFactorSearch(
  factorRanges: { name: string; min: number; max: number }[],
  goal: OptimizationGoal = 'maximize',
): MultiFactorSearchState {
  const factors: Record<string, GoldenSectionState> = {};
 ...
 */
export function advanceMultiFactorSearch(
  state: MultiFactorSearchState,
  x: number,
  y: number,
  goal: OptimizationGoal = 'maximize',
): MultiFactorSearchState {
  if (state.completed || !state.currentFactor) return state;

  const newState = { ...state, factors: { ...state.factors } };
  const current = newState.currentFactor!; // 上面已检查 non-null

  // 推进当前因素
  newState.factors[current] = advanceGoldenSearch(
    state.factors[current], x, y, goal
  );
