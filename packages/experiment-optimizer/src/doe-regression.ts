...
export interface RegressionResult {
  ...
  modelType: string;
  /** 回归系数 (含截距) */
  coefficients: { term: string; value: number; stdError: number; tValue: number; pValue: number; significant: boolean }[];
  /** 截距 */
  intercept: number;
  /** 决定系数 R² */
  rSquared: number;
...
export function polynomialRegression(
  ...
  const points: RegressionPoint[] = xValues.map((x, i) => {
    const xFeatures: Record<string, number> = {};
    for (let d = 1; d <= degree; d++) {
...
export function multipleRegression(
  ...
  if (n < p) return null; // 数据不足

  // 构造设计矩阵 X (含截距列)
  const X: number[][] = points.map(pt => {
    const row = [1]; // 截距
 ...
 * @param xNames - 原始自变量名
 */
export function quadraticSurfaceRegression(
  points: RegressionPoint[],
  xNames: string[],
): RegressionResult | null {
  // 扩展特征: 线性 + 二次 + 交互
  ...
  // 扩展数据点
  const expandedPoints: RegressionPoint[] = points.map(pt => {
...
function buildEquationString(
  coefficients: { term: string; value: number; significant: boolean }[],
): string {
  const parts: string[] = [];
 ...
 * 残差诊断分析
 */
export function diagnoseResiduals(result: RegressionResult): ResidualDiagnostics {
