...
export interface TermCoefficient {
  term: string;          // 'intercept' | 'A' | 'A:B' | 'A^2'
  coefficient: number;
  std_error?: number;
  t_statistic?: number;
  p_value?: number;
 ...
 */
export function buildTermList(factorNames: string[], type: ModelType): string[] {
  const main = factorNames.map(n => n);
  if (type === 'linear') return main;
  ...
  if (type === 'interaction') return [...main, ...interactions];
 ...
 */
function expandFeatures(coded: number[], type: ModelType): number[] {
  const row = [1, ...coded]; // intercept + main
  if (type === 'linear') return row;
  // interactions
  ...
  if (type === 'interaction') return row;
  // quadratic
  for (let i = 0; i < coded.length; i++) {
...
export function fitResponseModel(
  ...
  if (points.length === 0 || factors.length === 0) return null;

  // 构造 X (n × p), y (n)
  const X: number[][] = points.map(p => expandFeatures(codedVector(p.factor_values, factors), type));
  const y: number[] = points.map(p => p.response);
  const n = X.length;
  const p = X[0].length;
  ...
  const terms: TermCoefficient[] = beta.map((coef, i) => {
...
export function findOptimum(
  ...
  // 递归笛卡尔积 (k 维, grid^k 点)
  let best: { vals: Record<string, number>; pred: number } | null = null;
  const iterate = (depth: number, current: Record<string, number>) => {
    if (depth === k) {
  ...
  return {
    optimum_factor_values: best ? (best as any).vals : {},
    predicted_response: best ? (best as any).pred : 0,
    goal,
  };
}

// ─── Pareto chart 数据 (绝对系数排序) ───────────────────────
...
export function paretoChart(model: ModelFitResult): ParetoEntry[] {
  return model.terms
    .map(t => ({
      term: t.term,
