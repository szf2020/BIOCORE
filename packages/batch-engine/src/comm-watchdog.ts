...
export interface CommWatchdogConfig {
  // 是否在通讯恢复时自动 Restart (默认 false, 需操作员手动确认)
  autoRestartOnRestore: boolean;
  // 断线后最大安全运行时间 (秒), 超过则触发 cmd_stop (默认 3600 = 1小时)
  maxSafeHoldDuration_s: number;
}

export class CommWatchdog extends EventEmitter {
  private plc: PLCConnectionManager;
  private config: CommWatchdogConfig;
  private holdStartTime: number | null = null;
  private safetyTimer: ReturnType<typeof setTimeout> | null = null;
  private commLost = false;
