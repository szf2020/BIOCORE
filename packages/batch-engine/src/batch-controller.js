...
const step_engine_1 = require("./step-engine");
const running_fault_monitor_1 = require("./running-fault-monitor");
const recipe_validator_1 = require("./recipe-validator");
class BatchController extends events_1.EventEmitter {
    actor;
    stepEngine = null;
    faultMonitor = new running_fault_monitor_1.RunningFaultMonitor();
    config;
    recipe = null;
    batchId = '';
    phaseIndex = 0;
    ...
    constructor(config) {
        ...
        this.config = config;
        // 创建 XState actor
        this.actor = (0, xstate_1.createActor)(index_1.batchMachine);
    ...
    get buttons() {
        return (0, index_1.getButtonEnableState)(this.currentState);
    }
    // ── 命令接口 ──
    async start(recipe, batchId) {
        if (this.currentState !== 'idle') {
            return { success: false, message: `当前状态${this.currentState}不允许启动` };
        }
        // 配方校验
        const validation = (0, recipe_validator_1.validateRecipe)(recipe);
        if (!validation.valid) {
            return { success: false, message: `配方校验失败: ${validation.errors.map(e => e.message).join('; ')}` };
        }
        // Interlock检查
        const interlocks = await (0, index_1.checkInterlocks)(this.config.plcRead);
    ...
    startPhaseByIndex(phaseIndex) {
        ...
        if (!ps)
            return { success: false, message: 'Phase状态不存在' };
    ...
    /** 跳过指定Phase */
    skipPhase(phaseIndex) {
        const ps = this.phaseStatuses[phaseIndex];
        if (!ps || ps.state === 'completed' || ps.state === 'skipped')
            return;
    ...
    checkAllPhasesComplete() {
        const allDone = this.phaseStatuses.every(ps => ps.state === 'completed' || ps.state === 'skipped');
        if (allDone) {
            this.stopPolling();
     ...
     */
    resolveStepDefinitions(phaseType) {
        if (this.config.getTemplateSteps) {
            const dbSteps = this.config.getTemplateSteps(phaseType);
            if (dbSteps && dbSteps.length > 0)
                return dbSteps;
        }
        return (0, index_1.getStepDefinitions)(phaseType);
    }
    launchPhaseEngine(index) {
        if (!this.recipe || index >= this.recipe.phases.length)
            return;
        ...
        this.stepEngine = new step_engine_1.StepEngine(phase.type, index, phase.phase_id, phase.params || {}, templateSteps);
    ...
    async tickInternal() {
        if (this.currentState !== 'running' || !this.recipe)
            return;
        // 找到当前running的Phase
    ...
    async readProcessValues() {
        ...
        for (const tag of tags) {
            try {
                pv[tag] = await this.config.plcRead(tag);
            }
            catch { /* skip */ }
        }
