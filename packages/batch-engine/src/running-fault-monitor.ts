...
export interface FaultCheckResult {
  code: string;       // RF-01 ~ RF-11
  name: string;
  faulted: boolean;
  detail: string;
  severity: 'critical' | 'warning';
  holdAction?: string; // Hold时的附加动作
}
...
export class RunningFaultMonitor extends EventEmitter {
  private trackers = new Map<string, DurationTracker>();
  private lastValues = new Map<string, number>();
  private vfdConsecutiveSame = 0;
  private vfdLastValue: number | null = null;

  // 获取或创建持续计时器
  private track(code: string, condition: boolean): number {
  ...
  check(pv: Record<string, number>): FaultCheckResult[] {
    ...
    if (weight !== undefined && feedPump > 0) {
      ...
      // 每分钟更新基线
      if (noChangeDur === 0) this.lastValues.set('weight_for_rf09', weight);
    }
