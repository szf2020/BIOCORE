...
export const ALLOWED_OPS = ['>', '<', '>=', '<=', '==', '!='] as const;
...
export function parseExpression(input: string): ParseResult | ParseError {
  if (!input || typeof input !== 'string') return { ok: false, error: '表达式不能为空' };
  const trimmed = input.trim();
  if (trimmed.length > 500) return { ok: false, error: '表达式过长 (max 500)' };
  ...
  if (tokens.length === 0) return { ok: false, error: 'tokenize 失败' };

  // Simple recursive descent: 处理 a || b && c 这种结构
  // 优先级: && > ||
  let pos = 0;

  const parseComparison = (): ExprNode | null => {
    if (pos + 2 >= tokens.length + 1) return null;
    ...
    if (!ALLOWED_OPS.includes(op as any)) {
  ...
  const parseAnd = (): ExprNode | null => {
    ...
    while (pos < tokens.length && tokens[pos] === '&&') {
      ...
      if (!right) throw new Error('&& 右侧缺少表达式');
  ...
  try {
    const ast = parseOr();
    if (!ast) return { ok: false, error: '无法解析表达式' };
    if (pos < tokens.length) {
      return { ok: false, error: `解析后剩余多余 token: "${tokens.slice(pos).join(' ')}"` };
    }
    // 收集使用的字段
    const usedFields = new Set<AllowedField>();
    const walk = (n: ExprNode): void => {
      if (n.type === 'comparison') usedFields.add(n.field);
 ...
 */
export function evaluate(ast: ExprNode, context: Partial<Record<AllowedField, number>>): boolean {
  if (ast.type === 'comparison') {
    const lhs = context[ast.field];
    if (lhs === undefined || lhs === null || isNaN(lhs)) return false;
    switch (ast.op) {
    ...
    // 短路
    if (ast.op === '&&') return l && evaluate(ast.right, context);
...
export function evaluateExpression(
  input: string,
  context: Partial<Record<AllowedField, number>>,
