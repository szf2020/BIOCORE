...
class StepEngine extends events_1.EventEmitter {
    steps;
    currentIndex = 0;
    stepStartTime = Date.now();
    evaluator = new index_1.StepConditionEvaluator();
    phaseParams;
    phaseType;
    phaseIndex;
    phaseId;
    ...
    entrySnapshot = {};
    lastPV = {};
    constructor(phaseType, phaseIndex, phaseId, phaseParams = {}, 
    /** 外部注入的步骤定义 (来自数据库模板), 为空则回退硬编码 */
    injectedSteps) {
        super();
        const rawSteps = (injectedSteps && injectedSteps.length > 0)
            ? injectedSteps
            : (0, index_1.getStepDefinitions)(phaseType);
    ...
    applyParamsToConditions() {
        for (const step of this.steps) {
            ...
            if (this.phaseType === 'sip') {
                if (step.step_number === 2 && cond.value === undefined)
                    cond.value = this.phaseParams.target_temp_C ?? 121;
                if (step.step_number === 3 && cond.type === 'accumulated')
                    cond.value = this.phaseParams.hold_time_min ?? 20;
                if (step.step_number === 4 && cond.value === undefined)
                    cond.value = this.phaseParams.cool_to_C ?? 40;
            }
            // discharge: cool_to_C
            ...
            if (cond.type === '>=' || cond.type === '<=' || cond.type === 'in_band') {
                if (cond.value === undefined && cond.type !== 'in_band') {
                    console.warn(`[StepEngine] WARNING: ${this.phaseType} step ${step.step_number} ("${step.name}") ` +
    ...
    evaluate(pv) {
        ...
        const evalResult = this.evaluator.evaluate(stepDef.completion_condition, pv, this.stepStartTime, accumulated);
        if (evalResult.met) {
            // Step完成 → 记录日志
    ...
    createLogEntry(stepDef, elapsed, result, pv) {
        return {
...
//# sourceMappingURL=step-engine.js.map