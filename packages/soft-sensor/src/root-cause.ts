...
export interface AlarmAnalysisParams {
  alarmCode: string;
  alarmTime: Date;
  paramHistory: Record<string, number[]>; // 30min before/after
  paramNames: string[];
  normalRanges: Record<string, [number, number]>;
}

export interface AlarmAnalysisResult {
  probableCauses: { cause: string; confidence: number; evidence: string }[];
...
const ALARM_PATTERNS: Record<string, { param: string; direction: 'high' | 'low'; cause: string }[]> = {
  TEMP_HIGH: [
...
export class RootCauseAnalyzer {
  ...
  analyze(params: AlarmAnalysisParams): AlarmAnalysisResult {
    ...
    const affectedParams: string[] = [];
    const probableCauses: { cause: string; confidence: number; evidence: string }[] = [];

    // Step 1: Identify which parameters deviated from normal ranges
    const deviations: Record<string, { direction: 'high' | 'low'; severity: number; trend: string }> = {};
...
function describeTrend(values: number[]): string {
  ...
  if (relChange < 0.02) return 'stable';
  if (change > 0) return relChange > 0.2 ? 'rapidly increasing' : 'gradually increasing';
...
function generateRecommendation(
  ...
  deviations: Record<string, { direction: string; severity: number }>,
): string {
  if (causes.length === 0) {
...
function buildNarrative(
  alarmCode: string,
  alarmTime: Date,
  deviations: Record<string, { direction: string; trend: string }>,
