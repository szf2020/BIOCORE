...
export function useTagHistory(tagId: string, opts: UseTagHistoryOpts = {}): TagHistory {
  ...
  // Subtract 1 extra second so the oldest point generated by makeTimestamps
  // (anchor = Date.now() - count*intervalSec*1000) is always included even when
  // the two Date.now() calls are a few milliseconds apart.
  const cutoffMs = Date.now() - (windowSec + 1) * 1000;
