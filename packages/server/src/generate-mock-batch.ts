...
async function main() {
  ...
  // 确保有默认用户, 获取实际 user_id
  let actualOperatorId = OPERATOR_ID;
  const userRow = db.prepare('SELECT user_id FROM users WHERE user_id = ? OR username = ?').get(OPERATOR_ID, 'admin') as any;
  if (!userRow) {
    ...
    actualOperatorId = userRow.user_id;
    console.log(`  用户已存在: ${actualOperatorId}`);
  }
  ...
  if (!recipeExists) {
    console.log('  创建模拟配方...');
    db.prepare(`INSERT INTO recipes (recipe_id, version, name, author, target_organism, vessel_config, phases, status, created_by)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
      RECIPE_ID, RECIPE_VERSION, 'E.coli BL21 高密度发酵', actualOperatorId, ORGANISM,
  ...
  db.prepare(`INSERT INTO batches (batch_id, recipe_id, recipe_version, reactor_id, organism, operator_id,
    ...
    BATCH_ID, RECIPE_ID, RECIPE_VERSION, REACTOR_ID, ORGANISM, actualOperatorId,
  ...
  for (const s of samples) {
    insertSample.run(BATCH_ID, s.sample_time, s.elapsed_h, s.od600, s.glucose_g_L, s.acetate_g_L, s.product_titer, s.biomass_g_L, s.cell_viability_pct, actualOperatorId);
  }
