...
exports.VariableMappingManager = void 0;
...
class VariableMappingManager {
    ...
    migrate() {
        try {
            ...
            const hasS7Db = cols.some((c) => c.name === 's7_db');
            if (!hasS7Db) {
                this.db.exec("ALTER TABLE plc_connections ADD COLUMN s7_db INTEGER NOT NULL DEFAULT 1");
            }
        }
        catch (e) {
            console.warn(`[VariableMappingManager] migrate() failed: ${e.message}`);
        }
    }
    getConnections() {
    ...
    upsertConnection(c) {
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'))
    `).run(c.id, c.name, c.protocol, c.ip, c.port, c.rack, c.slot, c.s7_db ?? 1, c.serial_port, c.baudrate, c.parity, c.slave_id, c.heartbeat_write_address, c.heartbeat_read_address, c.heartbeat_timeout_ms, c.reconnect_interval_ms, c.enabled ? 1 : 0);
    }
    deleteConnection(id) {
    ...
    upsertVariable(v) {
        const addrCheck = (0, utils_1.validateAddr)(v.plc_address, v.data_type);
        if (!addrCheck.valid) {
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    `).run(v.id, v.tag_name, v.description, v.plc_address, v.data_type, v.direction, v.scaling_enabled ? 1 : 0, v.raw_min, v.raw_max, v.eng_min, v.eng_max, v.eng_unit, v.group, v.poll_rate_ms, v.enabled ? 1 : 0, v.connection_id);
    }
    deleteVariable(id) {
        ...
        this.db.transaction(() => {
            for (const c of data.connections || []) {
                try {
                    ...
                    errors.push(`${c.name}: ${e}`);
                }
            }
            for (const v of data.variables || []) {
                try {
                    this.upsertVariable(v);
                    imported++;
                }
                catch (e) {
                    errors.push(`${v.tag_name}: ${e}`);
                }
            }
        })();
...
exports.VariableMappingManager = VariableMappingManager;
//# sourceMappingURL=variable-mapping.js.map