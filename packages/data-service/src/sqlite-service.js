...
class SQLiteService {
    db;
    constructor(dbPath = './data/biocore.db') {
    ...
    createBatch(batch) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(batch.batch_id, batch.recipe_id, batch.recipe_version, batch.reactor_id || 'F01', batch.organism || null, batch.operator_id, batch.total_phases);
    }
    // 安全: 白名单限制可更新的列名，防止 SQL 注入
    static BATCH_UPDATABLE = new Set([
    ...
    listBatches(limit = 50, offset = 0, reactorId) {
        // M2.3: 可选 reactor_id 过滤 (供多反应器对比的批次下拉用)
        if (reactorId) {
            return this.db.prepare('SELECT * FROM batches WHERE reactor_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?').all(reactorId, limit, offset);
        }
    ...
    writeStateTransition(entry) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(entry.batch_id, entry.from_state, entry.to_state, entry.event, entry.triggered_by, entry.phase_id || null, entry.step_number ?? null, entry.context ? JSON.stringify(entry.context) : null);
    }
    getStateTransitions(batchId) {
    ...
    writeAuditLog(log) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(log.batch_id || null, log.user_id, log.action, log.target_type, log.target_id || null, log.old_value || null, log.new_value || null, log.reason || null, log.ip_address || null, log.trace_id || null);
    }
    getAuditLogs(batchId, limit = 100) {
        if (batchId) {
    ...
    createAlarm(alarm) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(alarm.batch_id || null, alarm.alarm_code, alarm.severity, alarm.source, alarm.message, alarm.channel || null, alarm.pv_at_trigger ?? null, alarm.sv_at_trigger ?? null);
    ...
    getUnacknowledgedAlarms(batchId) {
        if (batchId) {
    ...
    writePhaseLog(log) {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(log.batch_id, log.phase_index, log.phase_id, log.phase_type, log.total_steps, log.entry_snapshot ? JSON.stringify(log.entry_snapshot) : null);
        return result.lastInsertRowid;
    }
    completePhaseLog(id, result, completedSteps, exitSnapshot) {
        this.db.prepare(`
    ...
    `).run(log.batch_id, log.phase_index, log.phase_id, log.phase_type, log.step_number, log.step_name, Math.round(log.elapsed_sec), log.elapsed_sec, log.result, log.condition_actual ?? null, log.entry_snapshot ? JSON.stringify(log.entry_snapshot) : null, log.exit_snapshot ? JSON.stringify(log.exit_snapshot) : null);
    }
    getPhaseLogs(batchId) {
        return this.db.prepare('SELECT * FROM phase_logs WHERE batch_id = ? ORDER BY phase_index').all(batchId);
    }
    getStepLogs(batchId, phaseIndex) {
    ...
    writeCommEvent(event) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(event.batch_id || null, event.connection_id, event.event_type, event.reason || null, event.pc_counter ?? null, event.plc_counter ?? null, event.downtime_s ?? null, event.auto_held ? 1 : 0);
    }
    // ─── 离线取样 ─────────────────────────────────────────────
    addOfflineSample(sample) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(sample.batch_id, sample.sample_time, sample.sampled_by, sample.od600 ?? null, sample.dcw_g_L ?? null, sample.glucose_g_L ?? null, sample.acetate_g_L ?? null, sample.product_titer ?? null, sample.product_unit || null, sample.lactate_g_L ?? null, sample.biomass_g_L ?? null, sample.cell_viability_pct ?? null, sample.ethanol_g_L ?? null, sample.extra_analytes ? JSON.stringify(sample.extra_analytes) : null, sample.notes || null);
    }
    getOfflineSamples(batchId) {
    ...
    createSuggestion(s) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(s.batch_id, s.suggestion_type, s.source_module, s.target_param, s.current_value ?? null, s.suggested_value ?? null, s.confidence ?? null, s.reasoning || null, s.expires_at || null);
        return result.lastInsertRowid;
    }
    ...
    rejectSuggestion(id, userId) {
        this.db.prepare(`
      UPDATE ai_suggestions SET status = 'rejected', decided_by = ?, decided_at = datetime('now')
      WHERE id = ? AND status = 'pending'
    `).run(userId, id);
    }
    expirePendingSuggestions(batchId) {
        this.db.prepare(`
      UPDATE ai_suggestions SET status = 'expired', decided_at = datetime('now')
      WHERE batch_id = ? AND status = 'pending'
    `).run(batchId);
    }
    getPendingSuggestions(batchId) {
        if (batchId) {
            return this.db.prepare("SELECT * FROM ai_suggestions WHERE batch_id = ? AND status = 'pending' ORDER BY created_at DESC").all(batchId);
        }
        return this.db.prepare("SELECT * FROM ai_suggestions WHERE status = 'pending' ORDER BY created_at DESC").all();
    }
    // ─── 配方 ─────────────────────────────────────────────────
    createRecipe(recipe) {
        // 序列化策略:
        ...
        if (parentVersion === undefined) {
            const prev = this.db.prepare('SELECT version FROM recipes WHERE recipe_id = ? ORDER BY created_at DESC LIMIT 1').get(recipe.recipe_id);
            if (prev)
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(recipe.recipe_id, recipe.version, recipe.name, recipe.author, recipe.target_organism || null, JSON.stringify(recipe.vessel_config), phasesJson, recipe.created_by, schemaVer, recipe.is_template ?? 0, recipe.parent_template_id || null, parentVersion || null);
    }
    approveRecipe(recipeId, version, approvedBy) {
    ...
    // M3.2: 提交审核
    submitForReview(recipeId, version) {
        this.db.prepare(`
    ...
    rejectRecipe(recipeId, version, reason) {
        if (!reason || !reason.trim())
            throw new Error('拒绝必须带原因');
        this.db.prepare(`
    ...
    countPendingApprovals() {
        const row = this.db.prepare("SELECT COUNT(*) AS cnt FROM recipes WHERE status = 'pending_approval' AND is_template = 0").get();
        return row?.cnt ?? 0;
    }
    // ── 配方废弃流程 ──
    // 提交废弃申请 (draft/approved → pending_deprecation)
    submitForDeprecation(recipeId, version) {
        this.db.prepare(`
    ...
    approveDeprecation(recipeId, version, approvedBy) {
        this.db.prepare(`
    ...
    rejectDeprecation(recipeId, version, reason) {
        if (!reason || !reason.trim())
            throw new Error('拒绝必须带原因');
        this.db.prepare(`
    ...
    // 从废弃恢复到草稿
    restoreDeprecated(recipeId, version) {
        this.db.prepare(`
    ...
    listRecipeVersions(recipeId) {
        return this.db.prepare(`SELECT recipe_id, version, status, created_at, created_by, parent_version, dag_schema_version, name
       FROM recipes
       WHERE recipe_id = ?
       ORDER BY created_at DESC`).all(recipeId);
    }
    // M3.1: 抓两个版本完整数据用于 diff
    getRecipeForDiff(recipeId, v1, v2) {
        const r1 = this.db.prepare('SELECT * FROM recipes WHERE recipe_id = ? AND version = ?').get(recipeId, v1);
        const r2 = this.db.prepare('SELECT * FROM recipes WHERE recipe_id = ? AND version = ?').get(recipeId, v2);
        if (!r1 || !r2)
            return null;
        return { v1: r1, v2: r2 };
    }
    ...
    saveAsTemplate(srcRecipeId, srcVersion, createdBy) {
        const src = this.db.prepare('SELECT * FROM recipes WHERE recipe_id = ? AND version = ?').get(srcRecipeId, srcVersion);
        if (!src)
            throw new Error(`源配方不存在: ${srcRecipeId}@${srcVersion}`);
    ...
    `).run(templateId, templateVersion, `${src.name}(模板)`, src.author, src.target_organism, src.vessel_config, src.phases, src.metadata, createdBy, src.dag_schema_version ?? 1);
        return { template_id: templateId, version: templateVersion };
    }
    // M3.3: 从模板创建一个新的实例配方
    // newRecipeId 由调用方决定 (避免命名冲突), parent_template_id 指向源
    instantiateTemplate(templateId, templateVersion, newRecipeId, newName, createdBy) {
        const tpl = this.db.prepare('SELECT * FROM recipes WHERE recipe_id = ? AND version = ? AND is_template = 1').get(templateId, templateVersion);
        if (!tpl)
            throw new Error(`模板不存在: ${templateId}@${templateVersion}`);
        this.db.prepare(`
      INSERT INTO recipes
        (recipe_id, version, name, author, target_organism, vessel_config, phases, metadata,
         status, created_by, dag_schema_version, is_template, parent_template_id, parent_version)
      VALUES (?, '1.0.0', ?, ?, ?, ?, ?, ?, 'draft', ?, ?, 0, ?, NULL)
    `).run(newRecipeId, newName, createdBy, tpl.target_organism, tpl.vessel_config, tpl.phases, tpl.metadata, createdBy, tpl.dag_schema_version ?? 1, templateId);
    }
    // ─── 校准 ─────────────────────────────────────────────────
    addCalibration(cal) {
        this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cal.channel, cal.sensor_type, cal.calibrated_by, cal.cal_point_low_raw ?? null, cal.cal_point_low_eng ?? null, cal.cal_point_high_raw ?? null, cal.cal_point_high_eng ?? null, cal.do_zero_offset ?? null, cal.do_slope ?? null, cal.do_barometric_mbar ?? null, cal.expires_at || null, cal.notes || null);
    }
    getLatestCalibration(channel) {
    ...
    static REACTOR_CATEGORIES = ['fermenter', 'bioreactor', 'centrifuge', 'purification', 'mixer', 'other'];
    listReactorConfigs() {
        return this.db.prepare('SELECT * FROM reactor_configs ORDER BY sort_order, reactor_id').all();
    }
    getReactorConfig(reactorId) {
    ...
    upsertReactorConfig(config) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
    `).run(config.reactor_id, config.name, config.description || '', config.vessel_volume_L ?? 5, config.plc_connection_id || null, config.plc_protocol || 's7', config.plc_ip || '192.168.2.1', config.plc_port ?? 102, config.plc_rack ?? 0, config.plc_slot ?? 1, config.heartbeat_write || 'VB400', config.heartbeat_read || 'VB401', config.enabled ?? 1, config.sort_order ?? 0, category);
    }
    deleteReactorConfig(reactorId) {
        this.db.prepare('DELETE FROM reactor_configs WHERE reactor_id = ?').run(reactorId);
    }
    // ─── 原料库 M9 (Sprint 2 M2.6) ──────────────────────────────
    // 原料类别白名单 (应用层校验)
    static RAW_MATERIAL_CATEGORIES = [
    ...
    listRawMaterials(opts = {}) {
        ...
        const rows = this.db.prepare(`SELECT * FROM raw_materials WHERE ${where.join(' AND ')} ORDER BY created_at DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
        // JSON 字段回解析
        return rows.map(r => ({
            ...r,
    ...
    getRawMaterial(materialId) {
        const row = this.db.prepare('SELECT * FROM raw_materials WHERE material_id = ? AND deleted_at IS NULL').get(materialId);
        if (!row)
            return null;
        return {
            ...row,
    ...
    `).run(m.material_id, m.name, m.category, m.supplier || null, m.catalog_no || null, m.unit || null, m.cost_per_unit ?? null, m.storage || null, m.physical_properties ? JSON.stringify(m.physical_properties) : null, m.notes || null, m.created_by || null);
    }
    updateRawMaterial(materialId, patch) {
        ...
        this.db.prepare(`UPDATE raw_materials SET ${fields.join(', ')} WHERE material_id = ? AND deleted_at IS NULL`).run(...values, materialId);
    }
    softDeleteRawMaterial(materialId) {
        this.db.prepare("UPDATE raw_materials SET deleted_at = datetime('now') WHERE material_id = ? AND deleted_at IS NULL").run(materialId);
    }
    setMsdsFilename(materialId, filename) {
        this.db.prepare("UPDATE raw_materials SET msds_filename = ?, msds_uploaded_at = datetime('now'), updated_at = datetime('now') WHERE material_id = ? AND deleted_at IS NULL").run(filename, materialId);
    }
    // ─── DoE 研究 CRUD ───────────────────────────────────────
    createDoeStudy(study) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?)
    `).run(study.study_id, study.name, study.description || null, study.base_recipe_id || null, study.base_recipe_version || null, study.design_type, JSON.stringify(study.factors), JSON.stringify(study.responses), study.created_by);
    }
    listDoeStudies() {
    ...
    getDoeStudy(studyId) {
        const row = this.db.prepare('SELECT * FROM doe_studies WHERE study_id = ?').get(studyId);
        if (!row)
            return null;
        return {
            ...row,
    ...
    updateDoeStudyStatus(studyId, status) {
        this.db.prepare("UPDATE doe_studies SET status = ?, updated_at = datetime('now') WHERE study_id = ?").run(status, studyId);
    }
    updateDoeStudy(studyId, patch) {
        const sets = [];
        const params = [];
        ...
        if (patch.design_type !== undefined) {
            sets.push('design_type = ?');
            params.push(patch.design_type);
        }
        ...
        if (patch.responses !== undefined) {
            sets.push('responses = ?');
            params.push(JSON.stringify(patch.responses));
        }
        if (patch.base_recipe_id !== undefined) {
            sets.push('base_recipe_id = ?');
            params.push(patch.base_recipe_id);
        }
        if (patch.base_recipe_version !== undefined) {
            sets.push('base_recipe_version = ?');
            params.push(patch.base_recipe_version);
        }
        if (sets.length === 0)
            return;
    ...
    replaceDoeRuns(studyId, rows) {
      ...
      `);
            for (const r of rows) {
                ins.run(`${studyId}-${r.run_index}`, studyId, r.run_index, JSON.stringify(r.factor_values));
            }
        });
        tx();
    ...
    listDoeRuns(studyId) {
        const rows = this.db.prepare('SELECT * FROM doe_runs WHERE study_id = ? ORDER BY run_index ASC').all(studyId);
        return rows.map((r) => ({
            ...r,
    ...
    getDoeRun(studyId, runIndex) {
        const row = this.db.prepare('SELECT * FROM doe_runs WHERE study_id = ? AND run_index = ?').get(studyId, runIndex);
        if (!row)
            return null;
        return {
            ...row,
    ...
    updateDoeRunRecipe(studyId, runIndex, recipeId, recipeVersion) {
        this.db.prepare(`
      UPDATE doe_runs
    ...
    bindDoeRunBatch(studyId, runIndex, batchId) {
        this.db.prepare(`
      UPDATE doe_runs
    ...
    setDoeRunResponse(studyId, runIndex, responses, notes) {
        this.db.prepare(`
      UPDATE doe_runs
