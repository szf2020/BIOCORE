...
> 版本：v2.0 · 日期：2026-05-14 · v2.0 同步至 v1.14.0 (2026-05-02 release)
...
| 3 | 自然语言 → InfluxDB Flux 查询 | `packages/ai-gateway/src/nl-to-flux.ts` | ✅ server `/ai/nl-to-flux` 已挂载，web-ui AI 页支持 |
| 4 | 批次自动摘要（完成后触发 LLM 归纳） | `packages/ai-gateway/src/batch-summary.ts:1-30` | ✅ 与 ai-report-routes 集成 |
| 5 | 上下文构建器（近 N 批次统计喂入 Prompt） | `packages/ai-gateway/src/context-builder.ts:1-40` | ✅ |
| 6 | 报告导出（HTML / DOCX / PDF） | `packages/ai-gateway/src/report-generator.ts` | 🟡 HTML 完整可用；PDF (puppeteer)、DOCX (docx 库) 为运行时动态加载的可选依赖，生产环境需手动确认已安装 |
...
| # | 功能 | 代码证据 | 状态 |
|---|---|---|---|
| 1 | 线性回归软测量（`SoftSensorEngine.predict`） | `packages/soft-sensor/src/index.ts:25-80` | ✅ OLS + 95% CI + 外推检测完整，server `/soft-sensor/predict|train|models` 已挂载，web-ui 已对接 |
| 2 | 置信区间与外推检测 | `packages/soft-sensor/src/index.ts:45-90`（`PredictionResult`） | ✅ |
| 3 | Monod 动力学补料建议 | `packages/soft-sensor/src/feed-advisor.ts`（`FeedAdvisor`） | ✅ server `/soft-sensor/feed-recommend` 已挂载 |
| 4 | 根因分析（异常参数回溯） | `packages/soft-sensor/src/root-cause.ts` | ✅ ALARM_PATTERNS 规则库完整，server `/root-cause/analyze` 已挂载 |
| 5 | 多反应器联合优化 | `packages/soft-sensor/src/multi-reactor.ts` | 🔵 当前为孤立库代码，server 用 batch-engine 的 ReactorManager，web-ui 无对应路由，设计冻结 |
...
- 配方审核队列（流程化管理）
- 原料追溯 + BOM（完整追踪链）
- 软测量线性回归推断（OLS + 95% CI + 外推检测）
- Monod 补料建议（server API 已就绪）
- 根因分析（ALARM_PATTERNS 规则库）

### 🟡 进行中（代码基本就绪，可选依赖或运行时约束）

- AI 报告 DOCX/PDF 导出（依赖 puppeteer / docx 库为可选，生产需手动确认安装）

### 🔵 v2 规划

- 贝叶斯 + 多保真度联合优化（实验设计智能化）
- ONNX 软测量推断（目前是线性回归，可升级深度学习）
- 多反应器联合优化（当前库代码孤立，设计冻结）
...
- `BIOCore_产品介绍.md` · `PRODUCT_OVERVIEW.md`
...
**架构与 API 文档：**
- `ARCHITECTURE.md` · `API_REFERENCE.md` · `WS_PROTOCOL.md`
- `部署说明.md`

**功能与配置指南：**
- `AI功能产品说明书.md`
- `B1-DAG-runtime-进度跟踪.md`（工作流 runtime）
- `SESSION_HANDOFF.md`（会话管理）

**发布记录（Release Notes）：**
- `RELEASE_NOTES/v1.6.0.md`
- `RELEASE_NOTES/v1.10.0.md` · `v1.11.0.md` · `v1.12.0.md` · `v1.13.0.md` · `v1.14.0.md`

**开发进度：**
- `开发进度_Sprint1_API公开化.md` · `Sprint2_价值增强.md` · `Sprint3_配方v2.md`
